// GrowMap v0.5 — Mobile-First Plant Explorer
// Features: Perenual + Trefle, images, 10K+ plants, mobile bottom sheet, geolocation
...
const App = {
    ...
    async init() {
        ...
        console.log('🚀 GrowMap v0.5 ready');
    },
        ...
        btn.addEventListener('click', () => {
            ...
            navigator.geolocation.getCurrentPosition(
                (pos) => {
                    ...
                    this.onLocationSelected(latitude, longitude, 'My Location');
                },
                (err) => {
                    btn.classList.remove('loading');
    ...
    // ==================== MOBILE: SEARCH BAR ====================
    initMobileSearch() {
        const input = document.getElementById('mobile-location-search');
        ...
        input.addEventListener('input', () => {
            ...
            debounceTimer = setTimeout(async () => {
                ...
                if (!results || results.length === 0) { suggestionsEl.classList.add('hidden'); return; }
                suggestionsEl.innerHTML = results.map(r => `
                    <div class="loc-suggestion-item" data-lat="${r.lat}" data-lng="${r.lng}" data-name="${r.display_name}">
                        <span class="loc-sug-icon">${this.getPlaceIcon(r.type)}</span>
                        <span class="loc-sug-text">${r.display_name}</span>
                    </div>
                `).join('');
    ...
    initBottomSheet() {
        ...
        const onEnd = () => {
            ...
            // Snap to states based on position
